...
- Null A: energy-consuming hash computation with no persistent consensus,
  settlement, issuance, or participant policy surface.
- Null B: solar heating of inert matter with no self-maintaining biochemical or
...
- `SHARED_STRUCTURE`: both targets pass all load-bearing gates, both nulls fail
  at least the agency or feedback gate for a non-definitional reason, and the
  common fields impose at least one constraint not supplied by ordinary causal
  description.
- `NARROWER_STRUCTURE`: a useful typed subset survives, or the structure works
  in one domain but not the other.
- `KILL`: the common packet is vacuous, metaphor-dependent, or unable to reject
  the nulls.
